...
// Create Supabase client (Singleton for general use, e.g., in utility functions or server-side search)
...
export async function deleteItemById(
  ...
  try {
    ...
    if (error) {
      console.error(
        `Error deleting item ${itemId} using provided client:`,
        error,
      );
      // Throw the original Supabase error for more details upstream
      // You might want to wrap it or just throw it directly
    ...
    console.log(
      `Successfully deleted item ${itemId} using request-scoped client.`,
    );
