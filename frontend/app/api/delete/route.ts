...
export async function POST(req: NextRequest) {
  ...
  if (sessionError) {
    console.error(
      "Session retrieval error in delete route:",
  ...
  try {
    ...
    if (fileName)
      console.log(`Associated filename for storage deletion: ${fileName}`);
    ...
    console.log(
      `Item Owner ID: ${itemOwnerId}, Current User ID: ${currentUserId}`,
    );
    ...
    if (!isAdmin && !isOwner) {
      console.log(
        `Authorization failed: User ${currentUserId} is not owner (${itemOwnerId}) and not admin.`,
      );
      return NextResponse.json(
    ...
    console.log(
      `Authorization successful: User is ${isAdmin ? "Admin" : isOwner ? "Owner" : "Unknown Role (but passed check)"}. Proceeding with delete.`,
    );
    ...
    if (fileName && typeof fileName === "string" && fileName.trim() !== "") {
      ...
      if (storageError) {
        ...
        console.error(
          `Storage delete error for file ${fileName}:`,
          storageError,
        );
